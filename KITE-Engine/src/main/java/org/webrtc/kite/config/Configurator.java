...
import com.cosmo.kite.usrmgmt.AccountCollection;
import com.cosmo.kite.usrmgmt.AccountManager;
import com.cosmo.kite.usrmgmt.AccountRole;
...
public class Configurator {
  ...
  public void buildConfig()
    ...
    this.name = "" + Utility.throwNoKeyOrBadValueException(jsonObject, "name", String.class, false);
    this.name = name.contains("%ts") ? name.replaceAll("%ts", "") + " (" + timestamp() + ")" : name;
    String reportPath = (String) Utility.throwNoKeyOrBadValueException(jsonObject, "reportFolder", String.class, true);
    ...
    List<JsonObject> appObjectList = (List<JsonObject>) Utility
      .throwNoKeyOrBadValueException(jsonObject, "apps", JsonArray.class, true);

    int size = browserObjectList.size() + (appObjectList != null ? appObjectList.size() : 0);
    ...
    if (accountFilePath != null) {
      AccountManager.getInstance().init(new AccountCollection(Utility.getJsonObject(accountFilePath)));
    }
    ...
    configHandler =
      new ConfigTypeOneHandlerWithGrid(permute, callbackURL, remoteObjectList, testObjectList,
  ...
  /**
   * Returns a JsonArrayBuilder based on the configObject list.
   *
   * @return JsonArrayBuilder configObject list json array
   */
  public JsonArrayBuilder getEndPointListJsonArray() {
    JsonArrayBuilder jsonArrayBuilder = Json.createArrayBuilder();
    for (EndPoint endPoint : this.configHandler.getEndPointList()) {
      jsonArrayBuilder.add(endPoint.getJsonObjectBuilder());
    }
